function gradX = ardjitkernelGradXaug(X,Y,Lambda,beta,sigma,spec,R)
% only do the first R of the specified dimensions
iw_sqrt = sqrt(Lambda);
N = size(X,1); Rtot = size(X,2);
if ~isempty(Y)&&spec == 1
    K = size(Y,1);
    ...
    %gradX = zeros(N*R,N*K);
    Xt = repmat(X(:,1:R),K,1);
    Yt = kron(Y(:,1:R),ones(N,1));
    vv = -repmat(KXY(:)',1,R).*(Xt(:)'-Yt(:)').*kron(Lambda(1:R),ones(1,K*N));
...
elseif ~isempty(Y)&&spec == 2
    ...
    Xt = repmat(X(:,1:R),K,1);
    Yt = kron(Y(:,1:R),ones(N,1));
    vv = -repmat(KXY(:)',1,R).*(Yt(:)'-Xt(:)').*kron(Lambda(1:R),ones(1,K*N));
...
%     end
    ...
    Xt11 = repmat(X(:,1:R),N,1);
    Xt12 = kron(X(:,1:R),ones(N,1));
    vv1 = -kron(Lambda(:,1:R),ones(1,N*N)).*(Xt11(:)'-Xt12(:)').*repmat(KXX(:)',1,R);
%     for r = 1:R
%         for l = 1:N
%             for i = 1:N
...
%     
    ...
    Xt21 = kron(X(:,1:R),ones(N,1));
    Xt22 = repmat(X(:,1:R),N,1);
    vv2 = -kron(Lambda(:,1:R),ones(1,N*N)).*(Xt21(:)'-Xt22(:)').*repmat(KXX(:)',1,R);
