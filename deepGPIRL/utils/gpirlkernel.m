...
d_uu = max(d_uu,0);
K_uu = rbf_var*exp(-0.5*d_uu).*nmat; %equ 5 in original paper
...
if nargin < 3,
    ...
    if gp.warp_x,
        ...
        % Noise is uniform.
        K_uf = nconst*rbf_var*exp(-0.5*d_uf); % in IRL log likelihood
    end;
else
...
try
    ...
    %alpha = inv(K_uu)*y, logDetAndPPrior = 0.5 det(K_uu)
catch err
    % Save dump.
    save dump_file_chol;
    % Display the error.
...
K_ufKinv = K_uf'*invK; %in IRL log likelihood
...
if nargout > 3,
    ...
    for i=1:dims,
        ...
        dhpdr(i,:) = -0.5*sum(bsxfun(@times,df.*K_uf,alpha),1); %dK_uf'*alpha
